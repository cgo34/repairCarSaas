    ...
    <v-container fluid>
      <v-row>
        ...
        >
          ...
          <template #top>
            <v-toolbar flat>
              <v-toolbar-title>Paramètres des Coefficients des Type de réparation</v-toolbar-title>
              <v-divider
                class="mx-4"
                inset
                vertical
                  ...
                  <v-card-text>
                    <v-container>
                      <v-row>
                        ...
                        >
                          <v-select
                            v-model="selectedSetting.repairTypes"
                            label="Select"
                            :items="repairTypes"
                            item-title="name"
                            item-value="id"
...
const useSettingPriceRepairTypeCoefficientState = container.get<IUseSettingPriceRepairTypeCoefficientState>(
  SYMBOLS.States.Setting.Price.RepairTypeCoefficientState
);
...
const {
  ...
  repairTypes
} = useSettingPriceRepairTypeCoefficientState;

const dialog = ref<boolean>(false);

const headers = [
  { title: 'Type de réparation', align: 'start', key: 'repairTypes.name' },
  { title: 'Coefficient', key: 'coefficient' },
  { title: 'Actions', sortable: false, key: 'actions' }
] as const;

const formTitle = computed(() =>
  selectedSetting.value?.repairTypeId ? 'Modifier le paramètre' : 'Nouveau paramètre'
);

const onEditBtnClick = (item: SettingPriceRepairTypeCoefficientViewModel) => {
  selectSetting(item);
  dialog.value = true;
};

const onDeleteBtnClick = (item: SettingPriceRepairTypeCoefficientViewModel) => {
  if (!item.repairTypeId) return;

  // TODO: Ajouter un dialogue de confirmation avant suppression
  deleteSetting(item.repairTypeId);
};
...
const onSaveEditDialogBtnClick = async () => {
  if (selectedSetting.value?.repairTypeId) {    
